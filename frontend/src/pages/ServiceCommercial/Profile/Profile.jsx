...
const ProfileService = () => {
    ...
    useEffect(() => {
        const fetchDevelopData = async () => {
            try {
                ...
                const response = await axios.post(`${url}/api/servicecomercial/findByName`, { email: userEmail });
                if (response.data.success) {
                    const servicecomercialData = response.data.data;
                    setData({
                        id: servicecomercialData._id,
                        name: servicecomercialData.name,
                        email: servicecomercialData.email,
                        password: servicecomercialData.password,
                        address: servicecomercialData.localisation,
                        phone: servicecomercialData.phone,
                        image: servicecomercialData.image // Set the image URL received from backend
                    });
                    localStorage.setItem("servicecomercialid", servicecomercialData._id);
                } else {
                    toast.error(response.data.message);
                }
            } catch (error) {
                console.error("Error fetching servicecomercial data:", error);
                toast.error("Error fetching servicecomercial data");
            }
        };
        fetchDevelopData();
    }, []);
    ...
    const handleUpdateClick = async () => {
        ...
        try {
            const response = await axios.put(`${url}/api/servicecomercial/upgrade/${data.id}`, formData);
            if (response.data.success) {
            ...
            console.error("Error updating servicecomercial data:", error.response?.data || error.message);
            toast.error("Error updating servicecomercial data");
        }
    };
