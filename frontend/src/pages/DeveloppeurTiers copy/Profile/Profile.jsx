...
const ProfileDevelop = () => {
    ...
    useEffect(() => {
        const fetchDevelopData = async () => {
            try {
                ...
                const response = await axios.post(`${url}/api/develop/findByName`, { email: userEmail });
                if (response.data.success) {
                    const developData = response.data.data;
                    setData({
                        id: developData._id,
                        name: developData.name,
                        email: developData.email,
                        password: developData.password,
                        address: developData.localisation,
                        phone: developData.phone,
                        image: developData.image // Set the image URL received from backend
                    });
                    localStorage.setItem("developid", developData._id);
                } else {
                    toast.error(response.data.message);
                }
            } catch (error) {
                console.error("Error fetching develop data:", error);
                toast.error("Error fetching develop data");
            }
        };
        fetchDevelopData();
    }, []);
    ...
    const handleUpdateClick = async () => {
        ...
        try {
            const response = await axios.put(`${url}/api/develop/upgrade/${data.id}`, formData);
            if (response.data.success) {
            ...
            console.error("Error updating develop data:", error.response?.data || error.message);
            toast.error("Error updating develop data");
        }
    };
